...
<!--          <div><strong>开放类型：</strong> {{ detailData.openType | examOpenType }}</div>-->
          <div><strong>开放类型：</strong>
            <dict-tag :options="dict.type.t_exam_open_type" :value="detailData.openType"/>
          </div>
...
import { createPaper } from '@/api/exam/paper/exam'

export default {
  dicts: ['t_exam_open_type'],
  data() {
    return {
      detailData: {},
      postForm: {
